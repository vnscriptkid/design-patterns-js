...
class LineToPointAdapter extends Array {
  ...
  constructor(line) {
    ...
    // todo: gen hash, check in cache?
    this.hash = line.toString().hashCode();

    if (!(this.hash in LineToPointAdapter.cache)) {
      console.log(
        `${LineToPointAdapter.count++}: Generating ` +
          `points for line ${line.toString()} (with caching)`
      );
      // how to share cache data accross object instances: static member of class
      // how to share count data accross object instances: static member of class
